    ...
    K: KeyType,
{
    ...
    pub fn with_capacity(capacity: usize) -> Result<NodeFile<K>> {
    ...
    K: KeyType,
{
    ...
    /// Get a key for the given node block and position
    pub fn get_key(&self, node_id: u64, i: usize) -> Result<K> {
        let view = self.get(node_id)?;
        let n: usize = view.num_keys().read() as usize;
        if i < n && i < MAX_NUMBER_KEYS {
            let offset = i * 8;
            let result: K = K::from_bytes(&view.keys().data()[offset..(offset + size_of::<K>())])?;
            Ok(result)
        } else {
    ...
    pub fn set_key(&mut self, node_id: u64, i: usize, key: K) -> Result<()> {
        ...
        if i <= n && i < MAX_NUMBER_KEYS {
            ...
            view.keys_mut().data_mut()[offset..(offset + size_of::<K>())].copy_from_slice(&key);
    ...
    pub fn split_child(
        ...
        for i in ((child_idx + 1)..=self.number_of_keys(parent_node_id)?).rev() {
            self.set_key(parent_node_id, i, self.get_key(parent_node_id, i - 1)?)?;
...
mod test {
    ...
    #[test]
    fn allocate_nodes() {
        let mut f: NodeFile<u64> = NodeFile::with_capacity(0).unwrap();
